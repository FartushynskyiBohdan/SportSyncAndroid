...
import { getAccountSettings, getCities, getCountries, getGenders, updateAccountSettings, updatePassword, getBlockedUsers, unblockUser } from '../../api/appApi';
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setCountries(c);
        if (s.country_id) {
          const cities = await getCities(s.country_id);
          setCities(cities);
        }
      } catch (err: any) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.root} showsVerticalScrollIndicator={false}>
      ...
      {activeSection === 'account' && settings && (
        <AccountEditForm settings={settings} genders={genders} countries={countries} cities={cities} setCities={setCities} onUpdate={() => {}} />
      )}
...
function AccountEditForm({ settings, genders, countries, cities, setCities, onUpdate }: any) {
  ...
  const handleCountryChange = async (newCountryId: string) => {
    ...
    if (newCountryId) {
      const cities = await getCities(Number(newCountryId));
      setCities(cities);
      setCityId('');
    }
  };
      ...
      [
        { text: 'Cancel' },
        {
          ...
          onPress: async () => {
            setDeleting(true);
            try {
              // Assuming deleteAccount is imported
              await (async () => {
